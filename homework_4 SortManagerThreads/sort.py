...
def main(folder):
    scan.scan(folder)
    for thread in scan.threads:   # become
        thread.join()

    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
        # Tuple with all filenames:
        tuple_images = tuple(
...
"""
1. Clear the picture folder
2. Copy test data from __picture to picture
3. Run the script: py sort.py picture/
"""