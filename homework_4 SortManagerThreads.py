...
def dir_creator(dirs_list, path_to_create):
    ...
    Takes two arguments, namely the list of dirs names and directory path for new directories creation.
    """
    ...
    if dirs_exists and path.is_dir():
        for fold_name in dirs_list:
            ...
            #  folder not exists and creates one if not
        ...
        if path.exists():
            if path.is_dir() and path.stem not in ('archives', 'video', 'audio', 'documents', 'images'):
                ...
                if path.name.startswith('.'):
                    ...
                    if path.suffix.upper() in ['.DOC', '.DOCX', '.TXT', '.PDF', '.XLSX', '.PPTX', '.IPYNB', '.ASICE',
                                               '.PY', '.INI', '.CFG', '.RST']:
                        src = path
...
def normalize(string_to_normalize):
    ...
    cyryllic_chars = '0123456789абвгдеёжзийклмнопрстуфхъыьэАБВГДЕЁЗИЙКЛМНОПРСТУФХЪЫЬЭ'
    ...
    for cyr_char in cyryllic_chars:
        for _ in latin_chars:
            correspondence_map[ord(cyryllic_chars[cyryllic_chars.index(
                cyr_char)])] = latin_chars[cyryllic_chars.index(cyr_char)]
...
def main():
    ...
    try:
        path = sys.argv[1]
        folder_to_check: Path = Path(path)
